...
func _on_mission_victory(_title: String) -> void:
	...
	dialogue_mgr.call("queue_message", "СашаV", "Сервер взломан! Архивы корпорации стёрты, а данные о поставках — у нас! Мы победили!", Color(0.3, 1.0, 0.4), 6.0)
	LogManager.info(">>> ПОДЗЕМНЫЙ МЕТРОПОЛИТЕН ПОЛНОСТЬЮ ЗАЧИЩЕН! <<<", "VICTORY")
...
func _setup_victory_panel_buttons() -> void:
	...
	btn_next.text = " ▶ СЛЕДУЮЩИЙ РАЙОН: ПЕНТХАУС ЦИТАДЕЛИ "
	...
	btn_next.pressed.connect(func() -> void:
		...
		if gm and gm.has_method("change_district"):
			gm.call("change_district", "citadel_penthouse", true)
		else:
...
func _spawn_road_exit_gates() -> void:
	...
	# 1. Северный портал в Пентхаус Цитадели (служебный лифт)
	var north_gate = Node3D.new()
	north_gate.set_script(gate_script)
	north_gate.name = "ExitGate_Citadel"
	north_gate.set("target_district_id", "citadel_penthouse")
	north_gate.set("gate_title", "ПЕНТХАУС ЦИТАДЕЛИ")
	north_gate.set("direction_hint", "Служебный скоростной лифт на вершину башни Синдиката")
	north_gate.position = Vector3(0, 0, -85)
	add_child(north_gate)

	# 2. Южный возврат в Старый район (выход на поверхность)
	...
	south_gate.set("direction_hint", "Подъем на поверхность в переулки Старого района")
	south_gate.position = Vector3(0, 0, 85)
